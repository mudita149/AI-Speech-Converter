...
function App() {
  // usestate to remember what user types like english text and if queue is empty
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      //Express API instantly replies with the database row ID
  ...
  const pollStatus = async (taskId) => {
    // clock that ticks every 2 seconds
    const interval = setInterval(async () => {
      try {
        //Ask the backend for all the queue tasks
        const res = await axios.get("/requests");
        const allTasks = res.data;

        // finding our specific task row in the database
  ...
  return (
    <div>
      <div className="flex flex-col items-center bg-[#FAFAFE] min-h-screen pb-16 relative overflow-hidden">
        ...
        {/* Z-INDEX WRAPPER SO UI SITS OVER THE GLOW */}
        <div className="relative z-10 w-full flex flex-col items-center">

          {/* NAVIGATION BAR */}
          <nav className="w-full flex justify-between items-center px-8 md:px-16 py-6 w-full max-w-7xl">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[#8B5CF6] rounded-lg shadow-sm flex items-center justify-center text-white text-lg font-bold">
                文A
              </div>
          ...
          <p className="mt-6 text-slate-500 text-center max-w-lg text-lg">
            Enter your text below and let our AI translate it to Hindi and generate natural-sounding speech
          </p>

          {/* THE MAIN TEXTBOX CARD (NOW ULTRA PREMIUM!) */}
          <div className="relative mt-12 w-[600px]">
            {/* Glowing Drop-Shadow behind the card! */}
            ...
            <div className="relative bg-white/50 backdrop-blur-3xl shadow-2xl rounded-3xl p-8 border border-white/80">
              <div className="bg-white/60 backdrop-blur-md rounded-xl p-4 border border-white shadow-inner">
                <textarea
                  value={text}
                  onChange={(e) => setText(e.target.value)}
                  placeholder="Type your English text here..."
                  className="w-full bg-transparent text-lg resize-none outline-none text-slate-700 min-h-[120px]"
                />
                {/* Word Counter */}
